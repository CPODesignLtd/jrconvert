...
if (!($p = mysql_connect('fssoftware.brn.savvy.cz:3306', 'savvy_mhdspoje', '13FO4mCL'))) {
  ...
  if ($_GET[typemap] == 1) {
    $sql = "select MAP, SKELETON_VEHICLE, SKELETON_USERNAME, SKELETON_TICKET from location
...
//    header("Location: http://skeleton.dpmdas.cz/Timetables/Timetables/DepartureMapForMobile?_ticket=5755CF4D-5D20-4A3F-AAFE-4A2A1546B5B3", true, 301);
  if ($_GET[typemap] == 1) {
    $tt = (($_GET[transporttype] != 'null') ? ("&TransportTypeName=" . $_GET[transporttype]):"");
    $res = $row[1] . "?_ticket=" . $row[3] . $tt . (($_GET[linename] != 'null') ? ("&LineName=" . $_GET[linename]):"") . "&style=position:absolute;width:100%;height:500px;";
  } else {
    $res = $row[1] . "?_ticket=" . $row[3] /*. "&Longitude=" . $x . "&Latitude=" . $y*/ . "&style=position:absolute;width:100%;height:500px;";//"http://skeleton.dpmdas.cz/Timetables/Timetables/DepartureMapForMobile?_ticket=5755CF4D-5D20-4A3F-AAFE-4A2A1546B5B3";
  }
//  }
}
