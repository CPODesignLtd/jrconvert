...
$mysqli = new mysqli('fssoftware.brn.savvy.cz', 'savvy_mhdspoje', '13FO4mCL', 'savvy_mhdspoje', 3306);
$mysqli->query("SET NAMES 'utf-8';");
$sql = "call getOdjezdyZastavkaSpoje(" . $location . " ," . $packet . ", " . $cas . ", " . $zastavka . ", " . $varGRF . ");";
//echo $sql;
...
$reshlavicka = $reshlavicka . "<th>" . iconv('windows-1250', 'UTF-8', $rs_linka);
$reshlavicka = $reshlavicka . "<th>" . iconv('windows-1250', 'UTF-8', $rs_odjezd);
$reshlavicka = $reshlavicka . "<th>" . iconv('windows-1250', 'UTF-8', $rs_smer);
$reshlavicka = $reshlavicka . "</tr>";
$reshlavicka = $reshlavicka . "<tr>";
...
while ($row = $result->fetch_row()) {
  if ($EXP[$row[0] . "_" . $row[4]] == null) {
    ...
    $res = $res . "<tr>";
    $res = $res . "<td style='text-align: right; width: 100%; padding: 0px 15px 0px 0px;'>";
    ...
    $res = $res . "<td style='padding: 5px 5px 5px 5px; width: auto; white-space: nowrap; font-weight: bold'>";
    $res = $res . (($spoj->HH < 10) ? "0" . $spoj->HH : $spoj->HH) . " : " . (($spoj->MM < 10) ? "0" . $spoj->MM : $spoj->MM);
    $res = $res . "</td>";

    $res = $res . "<td style='padding: 5px 15px 5px 5px; width: 100%; white-space: nowrap;'>";
