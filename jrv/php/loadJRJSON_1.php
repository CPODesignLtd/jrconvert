    ...
    if ($sdruzJR == FALSE) {
      for ($i = 0; $i < 24; $i++) {
        ...
        for ($ii = 0; $ii < $JRType->pocetsloupcu + 1; $ii++) {
          ...
          if ($JRType->odjezdy->elementAt($i)->elementAt($ii) != NIL) {
            $res = $res . (($JRType->odjezdy->elementAt($i)->elementAt($ii)->mm < 10) ? "0" . $JRType->odjezdy->elementAt($i)->elementAt($ii)->mm : $JRType->odjezdy->elementAt($i)->elementAt($ii)->mm) . (($showkurz == 1) ?  ' (' . $JRType->odjezdy->elementAt($i)->elementAt($ii)->chrono . ')': '');
            $res = $res . "&nbsp";
/*            $textpozn = FALSE;
            if ($JRType->odjezdy->elementAt($i)->elementAt($ii) != NIL) {
