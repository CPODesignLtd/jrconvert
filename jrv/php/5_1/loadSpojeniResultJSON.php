...
//  echo $sql . '</br>';
    ...
    if ($pocetCilu == 0) {
      $sql = "call getOdjezdyPrima(" . $location . ", " . $packet . ", " . ($H * 60 + $M) . ", " . /*($H * 60 + $M + $dobaSpoje)*/(-1) . ", " . $pocatek . ", " . $cil . ", " . $varGRF . ");";
//  echo $sql . '</br>';
      $mysqli = new mysqli('fssoftware.brn.savvy.cz', 'savvy_mhdspoje', '13FO4mCL', 'savvy_mhdspoje', 3306);
      $mysqli->query("SET NAMES 'utf-8';");
      $query1 = $mysqli->query($sql);
...
function sortSpoje($Spoje, $location, $packet, $casDO) {
...
$SpojeEliminate = new Vector();
    for ($i = 0; $i < $Spoje->size(); $i++) {
    if ($Spoje->elementAt($i)->CASdo <= $casDO) {
    $SpojeEliminate->addElement($Spoje->elementAt($i));
    }
    }
...
//      $resultText = getZastavkaText($SpojeEliminate->elementAt($i)->useky->elementAt($ii)->Zdo, $location, $packet);
      ...
      $SpojeEliminate->elementAt($i)->useky->elementAt($ii)->doLocA = $row[$SpojeEliminate->elementAt($i)->useky->elementAt($ii)->Zdo - 1][1]; //$resultText[1];
      $SpojeEliminate->elementAt($i)->useky->elementAt($ii)->doLocB = $row[$SpojeEliminate->elementAt($i)->useky->elementAt($ii)->Zdo - 1][2]; //$resultText[2];
    }
  }
...
$Spoje = sortSpoje($Spoje, $location, $packet, ($H * 60 + $M + $dobaSpoje));
...
$res = $res . "</div>";*/
$res = $res . "<table id='tablejrSeznam' class = 'tableSpojeni'>"; //style='max-width:700px; width: auto;'
$res = $res . "<tr>";
$res = $res . "<td>";

if (($Spoje != NULL) && ($Spoje->size() > 0)) {
  for ($i = 0; $i < $Spoje->size(); $i++) {
    $res = $res . "<table class = 'tableSpojeni1'>";//style='width: 100%;'
    $res = $res . "<tr>";
/*    $res = $res . "<th>" . iconv('ISO-8859-2', 'UTF-8', "Linka");
    ...
    for ($ii = 0; $ii < $Spoje->elementAt($i)->useky->size(); $ii++) {
      ...
      $res = $res . "<td class='td_linka'>";//style='text-align: right; width: auto; padding: 0px 15px 0px 0px;'
      $res = $res . "<span class = 'a_nazev_linky1' >";//style='font-size: 18px;'
      ...
      $res = $res . "<td class='td_zastaka1'>";//style='padding: 5px 15px 5px 5px; white-space: normal;'
      switch ($location) {
        case 17: {
      ...
      if ($ii == 0) {
        $res = $res . "<td class='td_t1'>";//style='padding: 5px 5px 5px 5px; white-space: normal; font-weight: bold'
      } else {
        $res = $res . "<td class='td_t2'>";//style='padding: 5px 5px 5px 5px; white-space: normal;'
      }
      $res = $res . (((($Spoj->CASod / 60) % 24) < 10) ? '0' . (($Spoj->CASod / 60) % 24) : (($Spoj->CASod / 60) % 24)) . " : " . ((($Spoj->CASod % 60) < 10) ? '0' . ($Spoj->CASod % 60) : ($Spoj->CASod % 60));
      $res = $res . "</td>";

      $res = $res . "<td class='td_zastavka2'>";//style='padding: 5px 15px 5px 5px; white-space: normal;'
      switch ($location) {
        case 17: {
      ...
      if ($ii == $Spoje->elementAt($i)->useky->size() - 1) {
        $res = $res . "<td class='td_t3'>";//style='padding: 5px 5px 5px 5px; white-space: normal; font-weight: bold'
      } else {
        $res = $res . "<td class='td_t4'>";//style='padding: 5px 5px 5px 5px; white-space: normal;'
      }
    ...
    $res = $res . "</table>";
    $res = $res . "<div class='div1'></div>";//style='margin-top: 20px;'
  }
} else {
  $res = $res . "<table class='tableSpojeniNone' >";//style='max-width:500px; width: auto;'
  $res = $res . "<tr>";
  $res = $res . "<td><a>";
  $res = $res . iconv('windows-1250', 'UTF-8', $rs_spojeninenalezeno) . $_GET['datum'] . ' -- '.$_GET['h'].":".$_GET['m'];
  $res = $res . "</a></td>";
  $res = $res . "</tr>";
  $res = $res . "</table>";
}
