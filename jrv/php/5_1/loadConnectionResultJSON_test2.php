...
function getDistance($loca_pocatek, $locb_pocatek) {
/*    global $LOCA_CIL;
    global $LOCB_CIL;

    if ($loca_pocatek < $locb_pocatek) {
        $pom = $loca_pocatek;
        $loca_pocatek = $locb_pocatek;
        $locb_pocatek = $pom;
    }

    $deltaA = ($LOCA_CIL - $loca_pocatek);
    $deltaB = ($LOCB_CIL - $locb_pocatek);
    echo "distance == " . sqrt($deltaA * $deltaA + $deltaB * $deltaB) * 1 . "<br>";
    return (sqrt($deltaA * $deltaA + $deltaB * $deltaB) * 1);*/
    return 0;
}
...
function getZASTAVKA($c_zastavky, $linka, $ctarif) {
    ...
    $res = (object) $ZASTAVKY[$c_zastavky];
//    }
...
//        echo "pocet pesich prestupu " . count($pesobus) . " od zastavky " . $od_zastavky . "</br>";
        if (count($pesobus) > 0) {
        ...
        $_ZASTAVKY_LINKY = (object) $ZASTAVKY_LINKY[$od_zastavky];
        if (count($_ZASTAVKY_LINKY) > 0) {
            foreach ($_ZASTAVKY_LINKY as $key_c_linky => $val) {
//                echo " 1. - " . $key_c_linky . "<br>";
                foreach (((object) $val) as $key_c_tarif => $val1) {
//                    echo " 2. - " . $key_c_tarif . "<br>";
                    $zastavka = (object) $val1;
                    //echo $zastavka->c_linky . ", " . $od_zastavky . ", " . $zastavka->c_tarif . "</br>";
                    $_LINKY_ZASTAVKY = (object) $LINKY_ZASTAVKY[$zastavka->c_linky];
                    foreach ($_LINKY_ZASTAVKY as $l_key_c_zastavky => $l_val) {
                        foreach ((object) $l_val as $l_key_c_tarif => $l_val1) {
                            $zastavka1 = (object) $l_val1;
...
//          echo "pesi linka == modif</br>";
                ...
                while (($odHH <= $doHH) /* && ($konec == false) */) {
                    for ($vGRF = 0; $vGRF < count($VYBER_VARGRF); $vGRF++) {
//                        echo count($SPOJE[$VYBER_VARGRF[$vGRF]]) . "<br>";
                        if ($SPOJE[$VYBER_VARGRF[$vGRF]][$c_linky][$smer][$odHH] != null) {
//                        if (($SPOJE[$c_linky][$smer][$odHH]->kodpozn & $varGRF > 0) || ($SPOJE[$c_linky][$smer][$odHH]->kodpozn == 0)) {
                            for ($i = 0; $i < count($SPOJE[$VYBER_VARGRF[$vGRF]][$c_linky][$smer][$odHH]); $i++) {
                                $pomSpojItem = (object) $SPOJE[$VYBER_VARGRF[$vGRF]][$c_linky][$smer][$odHH][$i];
//                                echo "kod pozn: " . $pomSpojItem->kodpozn . " , vargrf =  " . $varGRF . " | " . ($pomSpojItem->kodpozn & $varGRF) . "</br>";
...
//            echo " chrono : " . $pomSpojItem->chrono . "</br>";
                                if (($CHRONO[$c_linky][$smer][$pomSpojItem->chrono][$od_tarif] != null) && ($CHRONO[$c_linky][$smer][$pomSpojItem->chrono][$do_tarif] != null)) {
                                    ...
                                    /*                if (($c_linky == 9406) && ($do_zastavky == 22)) {
                                      ...
                                      if ($c_linky == 720) {
                                      echo $c_linky . " | " . $od_zastavky . " -> " . $do_zastavky . " : " . $startCAS . " -> " . $endCAS . " ( " . $doba_jizdy . " ) linka pred " . $linka_pred . " cas " . $cas . "</br>";
                                      } */
...
function getSpojeni($od_zastavky, $do_zastavky, $odcas) {
    ...
    while ($konec == false) {
        ...
        $start->prijezd = $cas;
        echo $cas . " - " . ($konec == true ? "true": "false") . "</br>";
        $start->vaha = 0;        
...
//                Hashtable</*TZastavka*/String, THrana> HranyList = getSpojeOdDo(String.valueOf(bod.id_zastavky), od_zastavky, do_zastavky, Day, bod.prijezd, SpojeniBody, CalendCode);
            ...
            /*      $mysqli = new mysqli('fssoftware.brn.savvy.cz', 'savvy_mhdspoje', '13FO4mCL', 'savvy_mhdspoje', 3306);
              ...
              //      echo $sql . "</br>";
              $result = $mysqli->query($sql); */
            getSpojeODDO($bod->id_zastavky, $od_zastavky, $do_zastavky, $bod->prijezd, $bod->Hrana->c_linky);
...
//                        echo "zastavka = " . getZASTAVKA($odZastavky->id_zastavky, null, null)->nazev_zastavky . "(" . getZASTAVKA($odZastavky->id_zastavky, null, null)->locA . " - " . getZASTAVKA($odZastavky->id_zastavky, null, null)->locB . ")" . "<br>";
//                        echo "dist " . getDistance(getZASTAVKA($odZastavky->id_zastavky, null, null)->locA, getZASTAVKA($odZastavky->id_zastavky, null, null)->locB) . "<br>";
                        $pomVaha = $pomHrana->doba_jizdy + $cekani + $odZastavky->vaha; // + getDistance(getZASTAVKA($odZastavky->id_zastavky, null, null)->locA, getZASTAVKA($odZastavky->id_zastavky, null, null)->locB);
                        if ($pomVaha + getDistance(getZASTAVKA($odZastavky->id_zastavky, null, null)->locA, getZASTAVKA($odZastavky->id_zastavky, null, null)->locB) < $doZastavky->vaha) {
...
/* $sql = "select od_zastavky, do_zastavky, cas, vzdalenost from pesobus
  ...
  //  echo $pPrestup->c_linky . " , " . $row[0] . " , " . $pPrestup->do_zastavky . " , " . $pPrestup->doba . " , " . $pPrestup->vzdalenost . "</br>";
  } */
...
$PESOBUS_ZASTAVKY = loadStructure($path, "pesobus.dat");

//echo "peso " . count($PESOBUS_ZASTAVKY) . "<br>";

foreach ($SPOJE as $key => $value) {
//    echo "key : " . $key . "<br>";
    if (($key & $varGRF > 0) || ($key == 0)) {
        $VYBER_VARGRF[count($VYBER_VARGRF)] = $key;
    }
}

/* for ($i = 0; $i < count($VYBER_VARGRF); $i++) {
  echo "kod pozn: " . $VYBER_VARGRF[$i] . " , vargrf =  " . $varGRF . " | " . ($VYBER_VARGRF[$i] & $varGRF) . "</br>";
  } */

$LOCA_CIL = getZASTAVKA($cil, null, null)->locA;
$LOCB_CIL = getZASTAVKA($cil, null, null)->locB;

if ($LOCA_CIL < $LOCB_CIL) {
    $pom = $LOCA_CIL;
    $LOCA_CIL = $LOCB_CIL;
    $LOCB_CIL = $pom;
}

//echo "cil = " . getZASTAVKA($cil, null, null)->nazev_zastavky . "(" . $LOCA_CIL . " - " . $LOCB_CIL . ")";
...
//      $res = $res . "</tr><tr>";
            ...
            for ($ii = 0; $ii < count($myarraylist[$i]->PartSpoje); $ii++) {
                ...
                if ($val1->typ_dopravy == 'P') {
                    if ($location == 6) {
                        $doprava = "<img id='sipkaRDown" . ($i * 100 + $ii) . "' class='walkshow' src = 'http://www.mhdspoje.cz/jrw50/image/sipkaRDown.png' onClick='showhideMap(map" . ($i * 100 + $ii) . ", \"divMap" . ($i * 100 + $ii) . "\", " . $zast_show->LOCB . ", " . $zast_show->LOCA . ", \"sipkaRDown" . ($i * 100 + $ii) . "\");' style='vertical-align: top !important;''></img><img src = 'http://www.mhdspoje.cz/jrw50/image/peso_small.png' style='vertical-align: top !important;''></img>";
                    } else {
                ...
                if ($location == 6) {
                    if ($val1->typ_dopravy == 'P') {
                        $res = $res . "<tr>";
                        $res = $res . "<td colspan='11'>";
                        $res = $res . "<div id='divMap" . ($i * 100 + $ii) . "' style='width: 100%; height: 300px; margin-top: 5px; margin-bottom: 5px;'>";
                        $res = $res . "</div>";
                        $resmap[count($resmap)] = "
                var directionsService" . ($i * 100 + $ii) . " = new google.maps.DirectionsService;
