...
if ($lang == 'cz') {
  $mesice = array(0 => "ledna", "února", "března", "dubna", "května", "června", "července", "srpna", "září", "října", "listopadu", "prosince");
}
if ($lang == 'sk') {
...
//    echo $con_server . "|" . $con_db . "|" . $con_pass;
  if (!($p1 = mysql_connect($con_server, $con_db, $con_pass))) {
...
function getNejblizsiOdjezd(&$TJRTypes, &$x, &$y, $sloupec, $odCasu, &$urcenipolohy) {
  global $xpoloha;
  global $ypoloha;
  $hh = $ypoloha = (integer) date("G");
  $mm = $xpoloha = (integer) date("i");
  $br = FALSE;
...
function getTrasaTable($Linka, $Trasa, $Tarif, $Smer, $Pasmo, $Poznamky, $Location, $Packet, $denniJR, $datumJR, $sdruzJR, $showpolohu, $urcenipolohy, $TChrono, $JRTypes, $TypeJR) {
  global $linka;
  global $showkurz;
  global $incspoje;
  global $xpoloha;
  ...
  $res = "";
  $res = $res . "<table class='t_in1'>";
  $res = $res . "<tr>";

  if (!isset($_GET['print'])) {
// --- ukazatel ---
        /*$res = $res . "<td class='tdheader'>";
      $res = $res . "<div class='div_ram_transparent'>";
      $res = $res . "&nbsp";
      $res = $res . "</div>";

      $res = $res . "<div class='div_ram_transparent'>";
      $res = $res . "<table class='t_in2'>";
      ...
      if (!isset($_GET['print'])) {
      $res = $res . "<td class='tdempty'>";
      } else {
      $res = $res . "<td class='td1' style='padding: 0 0 0 0;'>";
      }
      $res = $res . "&nbsp";
...
//    $res = $res . "<td class='cell_zastavky_pasmo' style='vertical-align:top;text-align:center'>";
    if (!isset($_GET['print'])) {
      $res = $res . "<td class='tdcenter' style='vertical-align:top;text-align:center'>";
    } else {
      $res = $res . "<td class='td2' style='vertical-align:top;text-align:center; padding: 0 0 0 0;'>";
    }
    $res = $res . "<div class='div_ram'>";
    $res = $res . "<label class='label'>" . iconv('UTF-8', 'UTF-8', "Pásmo") . "</label>";
    $res = $res . "</div>";
    ...
    for ($i = 0; $i < $Trasa->elementCount; $i++) {
      ...
      if (!isset($_GET['print'])) {
        ...
        $res = $res . "<td class='td3' style='padding-top: 3px; padding-bottom: 3px;'>";
      }
  ...
  // --- poloha ---
  if (($showpolohu == 1) /*&& ($urcenipolohy == TRUE)*/) {

/*    $odecet = 0;
    for ($i = 1; $i < $Trasa->elementCount; $i++) {
      if ($Trasa->elementAt($i)->stavi == TRUE) {
        if ($TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->chrono - 1)->chrono->elementAt($i)->doba_jizdy != -1) {
          if ($Smer == 0) {
            if ((integer) $Tarif >= (integer) $Trasa->elementAt($i)->Tarif) {
              $odecet = $odecet + $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->chrono - 1)->chrono->elementAt($i)->doba_jizdy;
            }
          } else {
            if ((integer) $Tarif <= (integer) $Trasa->elementAt($i)->Tarif) {
              $odecet = $odecet + $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->chrono - 1)->chrono->elementAt($i)->doba_jizdy;
    ...
    $startcas = ($ypoloha * 60) + $JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->mm - $odecet;
    $actual_hh = (integer) date("G");
    $actual_mm = (integer) date("i");
    $actual_time = ($actual_hh * 60) + $actual_mm;
...
/*    if ($debug == 1) {
      echo $JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt(0)->elementAt(0)->chrono - 1;
      echo $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt(0)->elementAt(0)->chrono - 1)->chrono->size()->elementAt(1)->doba_jizdy . ' ---<br>';
    }*/
//    if ($debug == 1) {
//      echo "debug mode<br>";
    for ($y = 0; $y < 24; $y++) {
      for ($x = 0; $x < $JRTypes->elementAt($TypeJR - 1)->pocetsloupcu + 1; $x++) {
        $odecet = 0;
    for ($i = 0; $i < $Trasa->elementCount; $i++) {
      if ($Trasa->elementAt($i)->stavi == TRUE) {
        if ($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x) != NIL) {
        if ($TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->chrono - 1)->chrono->elementAt($i)->doba_jizdy != -1) {
          if ($Smer == 0) {
            if ((integer) $Tarif >= (integer) $Trasa->elementAt($i)->Tarif) {
              $odecet = $odecet + $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->chrono - 1)->chrono->elementAt($i)->doba_jizdy;
            }
          } else {
            if ((integer) $Tarif <= (integer) $Trasa->elementAt($i)->Tarif) {
              $odecet = $odecet + $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->chrono - 1)->chrono->elementAt($i)->doba_jizdy;
    ...
    if ($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x) != NIL) {
    $startcas = ($y * 60) + $JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->mm - $odecet;
//    echo 'start cas > ' . (integer)($startcas / 60) . ':' . ($startcas % 60) . ' odecet ' . $odecet . ' startcas ' . $startcas .  '<br>';
    } else {
      $startcas = -1;
    }
        if ($startcas != -1) {
         $odecet = 0;
        for ($i = 0; $i < $Trasa->elementCount; $i++) {
//          echo $Trasa->elementAt($i)->Nazev . ' - ' . $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->chrono - 1)->chrono->elementAt($i)->doba_pocatek . '<br>';
//          if ($Trasa->elementAt($i)->stavi == TRUE) {
            if ($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x) != NIL) {
              if ($TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->chrono - 1)->chrono->elementAt($i)->doba_jizdy != -1) {
                $odecet = $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->chrono - 1)->chrono->elementAt($i)->doba_pocatek;//$odecet + $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($y)->elementAt($x)->chrono - 1)->chrono->elementAt($i)->doba_jizdy;
//                echo ($y * 60) + $JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->mm - $odecet . '<br>' ;
//                echo (integer)(($startcas + $odecet) / 60) . ':' . (($startcas + $odecet) % 60) . ' odecet ' . $odecet . '<br>';
                if (($startcas + $odecet) == $actual_time) {
                  $index_vozu[$i] = TRUE;
                }
/*                if (($y * 60) + $JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->mm - $odecet == $actual_time) {
                  $index_vozu[$i] = TRUE;
                }*/
              }
//            }
...
//    }
    ...
    for ($i = 0; $i < $Trasa->elementCount; $i++) {
      if ($TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->chrono - 1)->chrono->elementAt($i)->doba_jizdy != -1) {
        if ($i > 0) {
          $incmin = $incmin + $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($ypoloha)->elementAt($xpoloha)->chrono - 1)->chrono->elementAt($i)->doba_jizdy;
        }
        $cas_vozu = $startcas + $incmin;
        if ($cas_vozu <= $actual_time) {
          $index_vozu = $i;
        }
      }
    }*/
    ...
    $res = $res . "<div class='div_ram'>";
    $res = $res . "<label style='width: 37px;' class='label'>" . /*iconv('ISO-8859-2', 'UTF-8', "Poloha") .*/ "&nbsp;</label>";
    $res = $res . "</div>";
...
//      $res = $res . $actual_time . " | " . $startcas . " | " . ($startcas + $incmin) . " | " . $index_vozu . " | " . $i;
      if ($index_vozu[$i] != null) {
      if ($index_vozu[$i] == TRUE) {
        if ($Linka->doprava == 'T') {
          $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/image/autobus_small.png'>&nbsp;</img>";
        }
        if ($Linka->doprava == 'O') {
          $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/image/trolejbus_small.png'>&nbsp;</img>";
        }
        if ($Linka->doprava == 'A') {
            if ($Location == 17) {
                $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/image/plzen/tram.png'>&nbsp;</img>";
            } else {
                $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/image/tramvaj_small.png'>&nbsp;</img>";
            }
        }
        if ($Linka->doprava == 'L') {
          $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/image/lanovka_small.png'>&nbsp;</img>";
        }
      }
      }
...
//  $res = $res . "<td class = 'cell_zastavky_zastavka' style = 'vertical-align: top; text-align: center;'>";
  if (!isset($_GET['print'])) {
    ...
    $res = $res . "<td class='td4' style='vertical-align:top;text-align:center; padding:0 0 0 0;'>";
  }

  $res = $res . "<div class='div_ram'>";
  $res = $res . "<label class='label'>" . iconv('UTF-8', 'UTF-8', "Zastávka") . "</label>";
  $res = $res . "</div>";

  $res = $res . "<div class='div_ram_nobackground'>";
  if (!isset($_GET['print'])) {
    $res = $res . "<table class='t_in' id='zastavky'>";
  } else {
...
//            echo "onClick = '" . "getJR(" . $Linka->idlinky . ", " . $Smer . ", " . (integer) $Trasa->elementAt($i)->Tarif . ", " . $Location . ", " . $Packet . ", " . (($denniJR) ? '1' : '0') . ", \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", " . (($sdruzJR) ? '1' : '0') . ", null, null, null, 0);";
    ...
    if (!isset($_GET['print'])) {
      ...
      if ((integer) $Tarif == (integer) $Trasa->elementAt($i)->Tarif) {
        if ($Location == 17) {
            $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/css/Plzen/arrow_blue_right.png' >&nbsp;</img>";
        }
      }
      if (((integer) $Tarif < (integer) $Trasa->elementAt($i)->Tarif) && ($Smer == 0)) {
        if ($Trasa->elementAt($i)->stavi == TRUE) {  
        if ($Location == 17) {
            $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/css/Plzen/arrow_blue_down.png' >&nbsp;</img>";
        }
        }
      }
      if (((integer) $Tarif > (integer) $Trasa->elementAt($i)->Tarif) && ($Smer == 1)) {
        if ($Trasa->elementAt($i)->stavi == TRUE) {  
        if ($Location == 17) {
            $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/css/Plzen/arrow_blue_down.png' >&nbsp;</img>";
        }
      ...
      $res = $res . "<td class='td5' style='padding-top: 3px; padding-bottom: 3px;'>";
    }

    if (!isset($_GET['print'])) {
      $zast = "\"" . $Trasa->elementAt($i)->Nazev . (($Location == 17) ? iconv('UTF-8', 'UTF-8', ", Plzeň") : (($Location == 11) ? iconv('UTF-8', 'UTF-8', ", Opava") : (($Location == 5) ? iconv('UTF-8', 'UTF-8', ", Třebíč") : ""))) . "\"";
      switch ($Location) {
        case 17: {
            $res = $res . "<img src='//152.mhdspoje.cz/JRw50new/css/Plzen/ipd-station.png' onClick='if (event != null) { event.cancelBubble; event.stopPropagation(); } else { if (!event) var event = window.event; event.cancelBubble = true; event.returnValue = false; } selfobj.mapAllStops(" . $zast . ", " . (($Trasa->elementAt($i)->LocA == '') ? 'null' : $Trasa->elementAt($i)->LocA) . ", " . (($Trasa->elementAt($i)->LocB == '') ? 'null' : $Trasa->elementAt($i)->LocB) . ", " . (integer) $Trasa->elementAt($i)->ID . ");'>&nbsp;</img>";
            break;
          }
        case 1: {
    ...
    if ((integer) $Tarif == (integer) $Trasa->elementAt($i)->Tarif) {
      $res = $res . "<a class='nav' id='a_nazevzastavkyfocused'>" . $Trasa->elementAt($i)->Nazev . "&nbsp; &nbsp;</a>";
    } else {
      if ($Trasa->elementAt($i)->stavi == TRUE) {
        $res = $res . "<a class='nav' id='a_nazevzastavky'>" . $Trasa->elementAt($i)->Nazev . "&nbsp; &nbsp;</a>";
      } else {
        $res = $res . "<a class='nav' id='a_nazevzastavkydisabled'>" . $Trasa->elementAt($i)->Nazev . "&nbsp; &nbsp;</a>";
      }
    }
    ...
    for ($pki = 0; $pki < $Trasa->elementAt($i)->poznamky->size(); $pki++) {
      if ((integer) $Trasa->elementAt($i)->poznamky->elementAt($pki) != 0) {
        ...
        if ($Poznamky->elementAt((integer) $Trasa->elementAt($i)->poznamky->elementAt($pki))->pic == null) {
          ...
          $res = $res . "<img class='img_poznamka' src='../png/" . $Poznamky->elementAt((integer) $Trasa->elementAt($i)->poznamky->elementAt($pki))->pic . "' title = '" . $Poznamky->elementAt((integer) $Trasa->elementAt($i)->poznamky->elementAt($pki))->popis . "'></ing>";
        }
        $textpozn = TRUE;
      }
    }
...
//    $res = $res . "<div class = 'div_header_jr'>";
    ...
    $res = $res . "<label class='label' name='name_sloupec'>" . iconv('UTF-8', 'UTF-8', $JRType->popis) . "</label>";
    $res = $res . "</div>";
    ...
    if ($sdruzJR == FALSE) {
      for ($i = 0; $i < 24; $i++) {
        ...
        if ($i < 10) {
          $res = $res . "<label class='label' id='lhour'>" . "0" . $i . "</label>";
        } else {
          $res = $res . "<label class='label' id='lhour'>" . $i . "</label>";
        }
        $res = $res . "</td>";
        ...
        for ($ii = 0; $ii < $JRType->pocetsloupcu + 1; $ii++) {
          if ($ii == $JRType->pocetsloupcu) {
            ...
            $res = $res . "<td class='td100' style='width:100%'>";
          } else {
            if (($TypeJR == $JRType->sloupec) && ($i == $Y) && ($ii == $X) && (($X != NULL) || ($Y != NULL))) {
              if (!isset($_GET['print'])) {
              $res = $res . "<td class='background-primary' id='time_jr_active'>"; //$res = $res . "<td class='td_time_jr_active' id='time_jr_active'>";
              } else {
                $res = $res . "<td class='td_time_jr' id='time_jr_active'>";
              }
            } else {
              if ($JRType->odjezdy->elementAt($i)->elementAt($ii) != NIL) {
                $res = $res . "<td class='td_time_jr' id='time_jr' onClick = '" . "getJR(\"" . $Linka->idlinky . "\", " . $Smer . ", " . $Tarif . ", " . $Location . ", " . $Packet . ", " . (($denniJR) ? '1' : '0') . ", \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", " . (($sdruzJR) ? '1' : '0') . ", " . $JRType->sloupec . ", " . $ii . ", " . $i . ", 0, 0, 0, " . $showkurz . ", " . $incspoje . ");'>";
              } else {
                $res = $res . "<td>";
              }
            }
...
//            $res = $res . "&nbsp";
            /*            $textpozn = FALSE;
              ...
              $res = $res . "<img id = 'img_poznamka' class = 'img_poznamka' src = '../png/" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->elementAt($pki))->pic . "' title = '" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->elementAt($pki))->popis . "'></ing>";
              }
              $textpozn = TRUE;
              }
              } */
            ...
            for ($pki = 0; $pki < $JRType->odjezdy->elementAt($i)->elementAt($ii)->timepozn->size(); $pki++) {
              ...
              /*                if ((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->timepozn->elementAt($pki) != 0) {
                ...
                $res = $res . "<img id = 'img_poznamka' class = 'img_poznamka' src = '../png/" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->timepozn->elementAt($pki))->pic . "' title = '" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->timepozn->elementAt($pki))->popis . "'></ing>";
                }
                $textpozn = TRUE;
                } */
            }
            ...
            if ($JRType->odjezdy->elementAt($i)->elementAt($ii) != NIL) {
              for ($pki = 0; $pki < $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->size(); $pki++) {
                if ((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->elementAt($pki) != 0) {
                  ...
                  if ($Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->elementAt($pki))->pic == null) {
                    if (($TypeJR == $JRType->sloupec) && ($i == $Y) && ($ii == $X) && (($X != NULL) || ($Y != NULL))) {
                      $res = $res . "<a class='a_pozn_active' title = '" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->elementAt($pki))->popis . "'>";
                      $res = $res . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->elementAt($pki))->zkratka;
                      $res = $res . "</a>";
                    } else {
                      $res = $res . "<a class='a_pozn' title = '" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->poznamky->elementAt($pki))->popis . "'>";
              ...
              /*              $textpozn = FALSE;
                ...
                $res = $res . "<img id = 'img_poznamka' class = 'img_poznamka' src = '../png/" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->otherpozn->elementAt($pki))->pic . "' title = '" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->elementAt($ii)->otherpozn->elementAt($pki))->popis . "'></ing>";
                }
                $textpozn = TRUE;
                }
                }
...
// -------- zahlavi c. spoje ----------
      ...
      for ($i = 0; $i < $JRType->odjezdy->size() + 1; $i++) {
        if ($i == $JRType->odjezdy->size()) {
          ...
          $res = $res . "<td class='cell_time_jr_zahlavi' title = '" . iconv('UTF-8', 'UTF-8', "spoj číslo : ") . $JRType->odjezdy->elementAt($i)->cspoje . "'>";
...
//                $res = $res . "<img class = 'img_poznamka' src = '../png/" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->poznamky->elementAt($pki))->pic . "' title = '" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->poznamky->elementAt($pki))->popis . "'></ing>";
                $res = $res . "<img class = 'img_poznamka' src = '../png/" . $Poznamky->elementAt((integer) $JRType->odjezdy->elementAt($i)->poznamky->elementAt($pki))->pic . "'></ing>";
              }
              $textpozn = TRUE;
            }
          }
...
//    $res = $res . "<a class = 'a_poznamky_header'>";
    $res = $res . iconv('UTF-8', 'UTF-8', $rs_pozn_spoje);
//    $res = $res . "</a>";
//    $res = $res . "</div>";
    ...
    for ($i = 0; $i < $PoznamkySpoje->size(); $i++) {
      $res = $res . "<tr>";
      $res = $res . "<td class='td6' style='padding-left: 10px;'>";
...
//        $res = $res . "&nbsp";
        $res = $res . "<img class = 'img_poznamka' src = '../png/" . $Poznamky->elementAt((integer) $PoznamkySpoje->elementAt($i))->pic . "'></ing>";
      }
//      $res = $res . "</div>";
      $res = $res . "</td>";
//      if (!isset ($_GET['print'])) {
      $res = $res . "<td class='td7' style='white-space: normal; padding-left:10px;'>";
//      } else {
...
//    $res = $res . "<div class = 'div_poznamky_header'>";
//    $res = $res . "<a class = 'a_poznamky_header'>";
    $res = $res . iconv('UTF-8', 'UTF-8', $rs_pozn_zast);
//    $res = $res . "</a>";
//    $res = $res . "</div>";
    ...
    for ($i = 0; $i < $PoznamkyZastavky->size(); $i++) {
      $res = $res . "<tr>";
      $res = $res . "<td class='td8' style='padding-left: 10px;'>";
...
//        $res = $res . "</a>";
        ...
        $res = $res . "<img class = 'img_poznamka' src = '../png/" . $Poznamky->elementAt((integer) $PoznamkyZastavky->elementAt($i))->pic . "'></ing>";
      }
//      $res = $res . "</div>";
      $res = $res . "</td>";


      $res = $res . "<td class='td9' style='white-space: normal; padding-left:10px;'>";
...
function getJizdaTable($Linka, $Trasa, $TChrono, $Tarif, $Smer, $Pasmo, $Poznamky, $Location, $Packet, $JRTypes, $TypeJR, $X, $Y) {
  ...
  /*if ($debug == 1) {
    echo 'pocet chrono : ' . $TChrono->elementAt($JRTypes->elementAt($TypeJR - 1)->odjezdy->elementAt($Y)->elementAt($X)->chrono - 1)->chrono->elementAt(1)->doba_jizdy . '<br>'  ;
  }*/
...
//  $res = $res . "<a id = 'header_zastavky_pasmo' class = 'header_zastavky_pasmo'>";
  $res = $res . "<label class='label'>" . iconv('UTF-8', 'UTF-8', "Min") . "</label>";
//  $res = $res . "</a>";
  $res = $res . "</div>";
...
// --- Čas ---
  ...
  $res = $res . "<div class = 'div_ram'>";
//  $res = $res . "<a id = 'header_zastavky_pasmo' class = 'header_zastavky_pasmo'>";
  $res = $res . "<label class='label'>" . iconv('UTF-8', 'UTF-8', "Čas") . "</label>";
//  $res = $res . "</a>";
  $res = $res . "</div>";
...
// --- cena ---
  /* if ($location == 6) {
    ...
    $res = $res . "<div class = 'div_ram'>";
    //  $res = $res . "<a id = 'header_zastavky_pasmo' class = 'header_zastavky_pasmo'>";
    $res = $res . "<label class='label'>" . iconv('ISO-8859-2', 'UTF-8', "Cena") . "</label>";
    //  $res = $res . "</a>";
    $res = $res . "</div>";
...
//  $res = $res . "<a id = 'header_zastavky_zastavka' class = 'header_zastavky_zastavka'>";
  $res = $res . "<label class='label'>" . iconv('UTF-8', 'UTF-8', $rs_jr_zastavka/* "Zastávka" */) . "</label>";
//  $res = $res . "</a>";
  $res = $res . "</div>";
...
//    $res = $res . "<td id = 'cell_zastavky_zastavka' class = 'cell_zastavky_zastavka'>";
    ...
    $addpasmo = "";
    if ($location == 6) {
      $addpasmo = " (&nbsp;" . (($smer == 0) ? $Trasa->elementAt($i)->PasmoA : $Trasa->elementAt($i)->PasmoB) . "&nbsp)";
      if (($smer == 0) && ($Trasa->elementAt($i)->PasmoA == '')) {
        $addpasmo = '';
      }
      if (($smer == 1) && ($Trasa->elementAt($i)->PasmoB == '')) {
        $addpasmo = '';
      }
    }
    $res = $res . $Trasa->elementAt($i)->Nazev . $addpasmo;
//    $res = $res . "</a>";
...
function getJRDiv($Linka, $Trasa, $TChrono, $Tarif, $Smer, $Pasmo, $Poznamky, $JRTypes, $Location, $Packet, $TypeJR, $X, $Y, $PoznamkyZastavky, $PoznamkySpoje, $denniJR, $datumJR, $sdruzJR, $layout, $Packets, $zastavkysmery, $showpolohu, $urcenipolohy) {
  global $showkurz;
  global $incspoje;
  global $popis1;
  global $popis2;
...
//printactive.png
    ...
    if ($zastavkysmery[$opaksmer][$Tarif] == 1) {
      $res = $res . "<a class='wclose' style='float:left; cursor:pointer;height:100%;display: block;vertical-align:middle;' title='" . iconv('UTF-8', 'UTF-8', $popis1) . "' src='http://www.mhdspoje.cz/jrw50/image/smer.PNG' onClick='" . "getJR(\"" . $Linka->idlinky . "\", " . $opaksmer . ", " . $Tarif . ", " . $Location . ", " . $Packet . ", " . (($denniJR) ? '1' : '0') . ", \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", " . (($sdruzJR) ? '1' : '0') . ", null, null, null, 0, 0, " . $showkurz . ", " . $incspoje . ");'>" . iconv('UTF-8', 'UTF-8', $popis2) . "</a>";
//      $res = $res . "<img class='wclose' style='float:left; cursor:pointer;' title='" . iconv('ISO-8859-2', 'UTF-8', "Zobrazit opačný směr") . "' src='http://www.mhdspoje.cz/jrw50/image/smer.PNG' onClick='" . "getJR(" . $Linka->idlinky . ", " . $opaksmer . ", " . $Tarif . ", " . $Location . ", " . $Packet . ", " . (($denniJR) ? '1' : '0') . ", \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", " . (($sdruzJR) ? '1' : '0') . ", null, null, null, 0, 0);'></img>";
    }
    }
    $res = $res . "</div>";
  }

  if (($Location == 6) || ($Location == 12) || ($Location == 11) || ($Location == 17)) {
    if (!isset($_GET['print'])) {
    $res = $res . "<table id='tablejrheader' class = 'tablejr' style='width: 100%;'>
<tbody>
<tr>
<td style='text-align: left;'><button title='' onclick='getJR(\"" . $Linka->idlinky . "\", " . $opaksmer . ", \"\", " . $Location . ", " . $Packet . ", " . (($denniJR) ? '1' : '0') . ", \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", " . (($sdruzJR) ? '1' : '0') . ", null, null, null, 0, 0, " . $showkurz . ", " . $incspoje . ");' id='mod-rscontact-submit-btn-124' name='mod_rscontact_submit-btn-124' class='formButton'><span class=''></span>" . iconv('UTF-8', 'UTF-8', $popis2) . "</button></td>";
if ($denniJR == 1) {
$res = $res . "<td style='text-align: right;'><button title='' onclick='printJRden(\"" . $Linka->idlinky . "\", " . $Smer . ", $Tarif, " . $Location . ", " . $Packet . ", " . (($denniJR) ? '1' : '0') . ", \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", 0, null, null, null, 0, false); ' id='mod-rscontact-submit-btn-124' name='mod_rscontact_submit-btn-124' class='formButton'><span class=''></span>" . iconv('UTF-8', 'UTF-8', $rs_tisk) . "</button>";
}
$res = $res . "</tr>
</tbody>
</table>
...
//    }
  ...
  $res = $res . "<table id='tablejr' class = 'tablejr' style='width: 100%;'>";
    ...
    $res = $res . "<table id='tablejr' class = 'tablejr' style='width: auto;'>";
  }
...
//--- header linka ---
      $res = $res . "<table class='table100' style = 'width: 100%;'>";
...
//      $res = $res . "<div class = 'div_header_linka'>";
      ...
      $res = $res . iconv('UTF-8', 'UTF-8', $popis3);
      $res = $res . "</a>";
...
//--- tělo JŘ ---
      ...
      $res = $res . "<table class = 'table_JR' style='width: 100%; height: auto;'>";
        ...
        $res = $res . "<table class = 'table_JR' style='width: auto; height: auto;'>";
      }
      $res = $res . "<tr>";

      $res = $res . "<td class = 'cell_zastavky' rowspan=" . count($layout[$Location]) . ">";
      $res = $res . getTrasaTable($Linka, $Trasa, $Tarif, $Smer, $Pasmo, $Poznamky, $Location, $Packet, $denniJR, $datumJR, $sdruzJR, $showpolohu, $urcenipolohy, $TChrono, $JRTypes, $TypeJR);
      $res = $res . "</td>";
...
//                  $res = $res . "<div class='Rtable Rtable--3cols Rtable--collapse'>";
...
//          $res = $res . "<td class = 'cell_jr' colspan=2>";      
            if ($layout[$Location][0] != null) {
                $res = $res . "<td class = 'cell_zastavky' colspan=2 style = 'width: " . (100 / $layout[$Location][0]) . "%'>";
            } else {          
                $res = $res . "<td class = 'cell_zastavky' colspan=2>";
            }
          if (!isset($_GET['print'])) {
            //$res = $res . "<div style='width: auto;'>";
//            $res = $res . "<div style='order:0;' class=''>";
...
//      for ($jri = $JRTypes->Size() - 2; $jri < $JRTypes->Size(); $jri++) {
            for ($jri = $lastindex; $jri < min($lastindex + $layout[$Location][$l], $JRTypes->Size()); $jri++) {
              if ($sdruzJR == FALSE) {
//                $res = $res . "<td class = 'cell_jr'>";
                  if ($layout[$Location][$l] != null) {
                    $res = $res . "<td class = 'cell_zastavky' style = 'width: " . (100 / $layout[$Location][$l]) . "%'>";  
                  } else {
      ...
      $res = $res . "<div class='background-primary' style='height: 2px; width: 100%'></div>";
      $res = $res . "<div class = 'div_FS'>";
      $res = $res . "<a class = 'a_FS'>";
      $res = $res . iconv('UTF-8', 'UTF-8', "JRw ver. 5.0. - SKELETON &reg FS software s.r.o.");
      $res = $res . "</a>";
      $res = $res . "</div>";
      ...
      if ($Location == 17) {
          $res = $res . "<tr>";
          $res = $res . "<td style='border-top-style: solid;
    border-top-width: 1px;
    border-top-color: #000000;
    padding-top: 5px;
    display: flex;
    font-size: 11px;'>";
          $res = $res . "<div style='white-space: nowrap;'>";
          $res = $res . "Vyhrazujeme si právo na změnu jízdního řádu.";
          $res = $res . "</div>";
          $res = $res . "<div style='text-align: right; width: 100%; white-space: nowrap;'>";
          $res = $res . "Jízdní řád linky pro období od 1.5.2019 do 30.6.2019</br> 
Platí od 1.5.2019 (označení verze jízdního řádu schválené dopravním úřadem).";
          $res = $res . "</div>";
          $res = $res . "</td>";                  
          $res = $res . "</tr>";          
      }
...
//      print_r($sql);
//      print_r($result);
      //$p1 = mysql_connect($con_server, $con_db, $con_pass);

      $p1 = new mysqli($con_server, $con_db, $con_pass, $con_db, 3306);
      while ($row = mysql_fetch_row($result)) {
      ...
      if ($TJRTypes->size() == 0) {
        $sloupec = new TJRTypesElement();
        $sloupec->sloupec = 1;
        $sloupec->popis = iconv('UTF-8', 'UTF-8', "Všední den");
        $sloupec->vargrf = new Vector();
        ...
        $sloupec = new TJRTypesElement();
        $sloupec->sloupec = 2;
        $sloupec->popis = iconv('UTF-8', 'UTF-8', "Sobota");
        $sloupec->vargrf = new Vector();
        ...
        $sloupec = new TJRTypesElement();
        $sloupec->sloupec = 3;
        $sloupec->popis = iconv('UTF-8', 'UTF-8', "Neděle, svátek");
        $sloupec->vargrf = new Vector();
      ...
      $sloupec = new TJRTypesElement();
      $sloupec->sloupec = 1;
      $sloupec->popis = date_format(new DateTime($datumJR), 'd') . ". " . iconv('UTF-8', 'UTF-8', $mesice[(integer) (date_format(new DateTime($datumJR), 'm') - 1)]) . " " . date_format(new DateTime($datumJR), 'Y');
      $sloupec->vargrf = new Vector();
    ...
    if (($datumJR != date('Y-m-d')) && ($denniJR == FALSE) && ($sdruzJR == FALSE)) {
      ...
      if ((is_null($x) == TRUE) || (is_null($y) == TRUE) || (is_null($jrtype) == TRUE)) {
        if (($denniJR == FALSE) && ($sdruzJR == FALSE)) {
          ...
          getNejblizsiOdjezd($TJRTypes, $x, $y, $actualJRType, TRUE, $urcenipolohy);
          $jrtype = $actualJRType;
//        $res .= "<a>po x=".$x.", y=".$y."</a><br/>";
//        $res .=  "<a>jrtype =".$jrtype.", next = " . $nextJRType . "</a><br/>";
          if ((is_null($x) == TRUE) || (is_null($y) == TRUE)) {
            getNejblizsiOdjezd($TJRTypes, $x, $y, $nextJRType, FALSE, $urcenipolohy);
            $jrtype = $nextJRType;
//              $res .= "<a>po x=".$x.", y=".$y."</a><br/>";
//    $res .=  "<a>jrtype =".$jrtype.", next = " . $nextJRType . "</a><br/>";
          }
        ...
        if ($denniJR == TRUE) {
          getNejblizsiOdjezd($TJRTypes, $x, $y, $jrtype, TRUE, $urcenipolohy);
          if ((is_null($x) == TRUE) || (is_null($y) == TRUE)) {
            getNejblizsiOdjezd($TJRTypes, $x, $y, $jrtype, FALSE, $urcenipolohy);
          }
        }
      }
    }
...
//    $res .=  "<a>jrtype =".$jrtype.", next = " . $nextJRType . "</a><br/>";
    ...
    if ($showpolohu == 1) {
      ...
      $sql = "select chronometr.chrono, chronometr.c_zastavky, chronometr.c_tarif,
                    ...
                    from chronometr  left outer join zaslinky on (chronometr.c_tarif = zaslinky.c_tarif and chronometr.c_linky = zaslinky.c_linky and zaslinky.idlocation = " . $location . " and zaslinky.packet = " . $packet . ")
                    where ((" . $smer . "=0 and zaslinky.zast_a = 1) or (" . $smer . "=1 and zaslinky.zast_b = 1)) and chronometr.c_linky = '" . $linka . "' and chronometr.smer = " . $smer . " and
                    chronometr.idlocation = " . $location . " and chronometr.packet = " . $packet;
      if ($smer == 0) {
        $sql = $sql . " order by chronometr.chrono, chronometr.c_tarif";
      } else {
        $sql = $sql . " order by chronometr.chrono, chronometr.c_tarif desc";
...
//    echo $showpolohu . " | " . $urcenipolohy . "-";
    $showpasmo = ($smer == 0) ? $hasPasmoA : $hasPasmoB;
    if ($location == 17) {
        $showpasmo = null;
    }
    $res = $res . getJRDiv($Linka, $Trasa, $TChrono, $tarif, $smer, $showpasmo, $TPoznamky, $TJRTypes, $location, $packet, $jrtype, $x, $y, $TPoznamkyZastavky, $TPoznamkySpoje, $denniJR, $datumJR, $sdruzJR, $layout, $Packets, $zastavkysmery, $showpolohu, $urcenipolohy);
    $res = $res . '</div>';
