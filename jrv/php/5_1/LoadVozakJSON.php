...
//      $resjr = $resjr . "</td>";
    ...
    if ($spoj == 0) {
      $reshlavicka = $reshlavicka . "<td id='cj' class='td_hlav_zastavka'>";
      $zast = "\"" . $zastavky[$row_spoje_odjezdy[19]][1] . (($location == 17) ? iconv('ISO-8859-2', 'UTF-8', ", Plzeň") : (($location == 11) ? iconv('ISO-8859-2', 'UTF-8', ", Opava") : (($location == 5) ? iconv('ISO-8859-2', 'UTF-8', ", Třebíč") : ""))) . "\"";
      $reshlavicka = $reshlavicka . "<img src='../css/praporB.png' onClick='selfobj.map(" . $zast . ", " . (($zastavky[$row_spoje_odjezdy[19]][2] == '') ? 'null': $zastavky[$row_spoje_odjezdy[19]][2]) . ", " . (($zastavky[$row_spoje_odjezdy[19]][3] == '') ? 'null': $zastavky[$row_spoje_odjezdy[19]][3]) . ", " . $zastavky[$row_spoje_odjezdy[19]][0] . ");'>&nbsp;</img>";
      $reshlavicka = $reshlavicka . iconv('UTF-8', 'UTF-8', $zastavky[$row_spoje_odjezdy[19]][5]);
      $reshlavicka = $reshlavicka . "</td>";
    }

    $resjr = $resjr . "<td id='cj' class='td_time'>";
              ...
              for ($pki = 28; $pki <= 36; $pki++) {
                if (($row_spoje_odjezdy[$pki] != 0) && ($PoznamkyList[$row_spoje_odjezdy[$pki]] != NULL)) {
                  ...
                  if ($PoznamkyList[$row_spoje_odjezdy[$pki]][5] == '') {
                    ...
                    $resjr = $resjr . "</a>";*/
                    $resjr = $resjr . "<img id = 'img_poznamka' class = 'img_poznamka' src = '../png/" . $PoznamkyList[$row_spoje_odjezdy[$pki]][5] . "' title = '" . $PoznamkyList[$row_spoje_odjezdy[$pki]][2] . "'></ing>";
                  }
                  $textpozn = TRUE;
                }
              }
...
//  $reshlavickalinka = $reshlavickalinka . "</td>";
  ...
  $reshlavickalinka = $reshlavickalinka . iconv('ISO-8859-2', 'UTF-8', "Přejezd na linku ") . iconv('UTF-8', 'UTF-8', $sql_linka[1]) . iconv('ISO-8859-2', 'UTF-8', " , Kurz ") . iconv('UTF-8', 'UTF-8', $sql_kurz[0]);
...
//    $res = $res . "<img class='wclose' style='float:left;' src='http://www.mhdspoje.cz/jrw50/image/printer_red.png' onClick='" . "printJR(" . $Linka->idlinky . ", " . $Smer . ", " . $Tarif . ", " . $Location . ", " . $Packet . ", " . (($denniJR) ? '1' : '0') . ", \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", " . (($sdruzJR) ? '1' : '0') . ", null, null, null, 0, 0);'></img>";
    ...
    $res = $res . "<table id='tablejr' class = 'tablejrvozak'>";
...
//--- header linka ---
          $res = $res . "<table class='table_header'>";// style = 'width: 100%;'

            $res = $res . "<tr>";

              $res = $res . "<td class = 'td_ram_nobackground1'>";// style = 'min-width: 70px;'
              ...
              $res = $res . "<td class = 'td_ram_nobackground2'>";// style = 'text-align: left; min-width: 70px; width: 100%'
/*                $res = $res . "<a class = 'a_smer_linky_label'>";
                $res = $res . iconv('ISO-8859-2', 'UTF-8', "Směr :");
                $res = $res . "</a>";
...
//2      $res = $res . "</tr>";
    ...
    for ($i = 0; $i < count($VybranePoznList); $i++) {
      $res = $res . "<tr>";
      $res = $res . "<td class='td_pozn'>";//style='padding-left: 10px;'
      if ($PoznamkyList[$VybranePoznList[$i]][5] == '') {
        $res = $res . $PoznamkyList[$VybranePoznList[$i]][1];
      } else {
        $res = $res . "<img class = 'img_poznamka' src = '../png/" . $PoznamkyList[$VybranePoznList[$i]][5] . "'></img>";
      }
      $res = $res . "</td>";
      $res = $res . "<td class='td_pozn1'>";//style='white-space: normal; padding-left:10px;
