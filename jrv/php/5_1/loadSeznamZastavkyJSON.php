...
if (!($p = mysql_connect($con_server, $con_db, $con_pass))) {
    ...
    /* $res = $res . "<div class='div_pozadikomplex' style='width: auto; min-width: 200px;'>";
      ...
      $res = $res . "</div>"; */
    $res = $res . "<table id='tablejrSeznam' class = 'tableseznamzastavky'>";
...
and zastavky.packet = zaslinky.packet) where zastavky.idlocation = " . $location . " and zastavky.packet = " . $packet . " and zaslinky.voz = 1  order by zastavky.nazev COLLATE utf8_czech_ci";
//FROM zastavky where idlocation = " . $location . " and packet = " . $packet . " order by c_zastavky";
    ...
    while ($rowzastavky = mysql_fetch_row($result)) {
        ...
        $res = $res . "<td class='td_zastavka'>"; //style='text-align: left; width: auto;'

        $zast = "\"" . $rowzastavky[1] . (($location == 17) ? iconv('UTF-8', 'UTF-8', ", Plzeň") : (($location == 11) ? iconv('UTF-8', 'UTF-8', ", Opava") : (($location == 5) ? iconv('UTF-8', 'UTF-8', ", Třebíč") : ""))) . "\"";
        switch ($location) {
            case 17: {
                    $res = $res . "<img src='../css/Plzen/ipd-station.png' onClick='selfobj.map(" . $zast . ", " . (($rowzastavky[2] == '') ? 'null' : $rowzastavky[2]) . ", " . (($rowzastavky[3] == '') ? 'null' : $rowzastavky[3]) . ", " . $rowzastavky[0] . ");'>&nbsp;</img>";
                    break;
                }
            case 1: {
                    $res = $res . "<img src='../css/praporR.png' onClick='selfobj.map(" . $zast . ", " . (($rowzastavky[2] == '') ? 'null' : $rowzastavky[2]) . ", " . (($rowzastavky[3] == '') ? 'null' : $rowzastavky[3]) . ", " . $rowzastavky[0] . ");'>&nbsp;</img>";
                    break;
                }
            case 11: {
                    $res = $res . "<img src='../css/praporR.png' onClick='selfobj.map(" . $zast . ", " . (($rowzastavky[2] == '') ? 'null' : $rowzastavky[2]) . ", " . (($rowzastavky[3] == '') ? 'null' : $rowzastavky[3]) . ", " . $rowzastavky[0] . ");'>&nbsp;</img>";
            ...
            default: {
                    $res = $res . "<img src='../css/praporB.png' onClick='selfobj.map(" . $zast . ", " . (($rowzastavky[2] == '') ? 'null' : $rowzastavky[2]) . ", " . (($rowzastavky[3] == '') ? 'null' : $rowzastavky[3]) . ", " . $rowzastavky[0] . ");'>&nbsp;</img>";
                    break;
                }
        }
        ...
        $res = $res . "<td class='td_linka'>"; //style='display: inline; width: auto;'
...
//     }
            ...
            if ($location == 17) {
                $res = $res . "<a class='a_linka' onClick = '" . "selfobj.changeZIndexJR(); getJR(\"" . $rowlinky[2] . "\", 0, " . (integer) $rowlinky[4] . ", " . $location . ", " . $packet . ", 0, \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", 0, null, null, null, null, 1);'>"
                        . (($rowlinky[5] == 'T') ? "<img class='a_linka_img' src = '../image/plzen/bus_small.png'></img>" :
                        (($rowlinky[5] == 'O') ? "<img class='a_linka_img' src = '../image/plzen/trol_small.png'></img>" :
                        (($rowlinky[5] == 'A') ? "<img class='a_linka_img' src = '../image/plzen/tram_small.png'></img>" : "")))
                        . '&nbsp;' . trim($rowlinky[0]) /* . " (" . $rowlinky[4] . ") " */ . "</a>";
            } else {
                $res = $res . "<a class='a_linka' onClick = '" . "selfobj.changeZIndexJR(); getJR(\"" . $rowlinky[2] . "\", 0, " . (integer) $rowlinky[4] . ", " . $location . ", " . $packet . ", 0, \"" . date_format(new DateTime($datumJR), 'd_m_Y') . "\", 0, null, null, null, null, 1);'>"
                        . (($rowlinky[5] == 'T') ? "<img class='a_linka_img' src = '../image/autobus_small.png'></img>" :
                        (($rowlinky[5] == 'O') ? "<img class='a_linka_img' src = '../image/trolejbus_small.png'></img>" :
                        (($rowlinky[5] == 'A') ? "<img class='a_linka_img' src = '../image/tramvaj_small.png'></img>" : "")))
                        . '&nbsp;' . trim($rowlinky[0]) /* . " (" . $rowlinky[4] . ") " */ . "</a>";
            }
            $poradi++;
//      $res = $res . "</td>";
