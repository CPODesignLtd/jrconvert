...
$mysqli = new mysqli('fssoftware.brn.savvy.cz', 'savvy_mhdspoje', '13FO4mCL', 'savvy_mhdspoje', 3306);
$mysqli->query("SET NAMES 'utf-8';");
$sql = "call getOdjezdyZastavkaCasy(" . $location . " ," . $packet . ", " . ($H * 60 + $M) . ", " . ($H * 60 + $M + $maxcas) . ", " . $pocatek . ", " . $varGRF . ", '" . date_format(new DateTime($datumJR), 'Y-m-d') . "');";
echo "</br>call getOdjezdyZastavkaCasy(" . $location . " ," . $packet . ", " . ($H * 60 + $M) . ", " . ($H * 60 + $M + $maxcas) . ", " . $pocatek . ", " . $varGRF . ", '" . date_format(new DateTime($datumJR), 'Y-m-d') . "');</br>";
$result = $mysqli->query($sql);

$CASYODJEZDU = array(); 
while ($row = $result->fetch_row()) {
    $CASYODJEZDU[count($CASYODJEZDU)] = $row[0];
//    echo count($CASYODJEZDU) - 1 . "." . " " . $CASYODJEZDU[count($CASYODJEZDU) - 1] . " - " . $row[0] . "</br>";
}
...
function vypocet($od_zastavky, $do_zastavky, $odcas) {
...
function getSpojeni($od_zastavky, $do_zastavky, $odcas) {
    ...
    while (($konec == false) && ($icasy < count($CASYODJEZDU))) {//test na jeden spoj
//    for ($icasy = 0; $icasy < count($CASYODJEZDU); $icasy++) {
        $cas = $CASYODJEZDU[$icasy++];
        ...
        $bod = getSpojeniMinimum();  //TZastavka
        ...
        while (($bod != null) && ($bod->odZastavky != null)) {
            ...
            if ((count($myarraylist[$cislo_spoje]->PartSpoje) > 0) && ($myarraylist[$cislo_spoje]->PartSpoje[0]->c_linky == $bod->Hrana->c_linky) && ($myarraylist[$cislo_spoje]->PartSpoje[0]->smer == $bod->Hrana->id_smeru)) {
                ...
                $myarraylist[$cislo_spoje]->PartSpoje[0] = $obj;
            }
...
if (count($myarraylist) > 0) {
    for ($i = 0; $i < count($myarraylist); $i++) {
        $res = $res . "<tr><td colspan='11'><div style='margin-top: 20px; border-bottom: 1px solid #dadada;'></div></td></tr>";
        $res = $res . "<tr>";
...
echo "</br></br>" . (round(microtime(true) * 1000) - $sMil);

class AsyncOperation extends Thread {

    public function __construct($arg, &$finish) {
        $this->arg = $arg;
        $this->arg_finish = $finish;
    }
...
foreach ( range("A", "D") as $i ) {
    $front = new AsyncFront();
    $front->Operation = new AsyncOperation($i, $front);
    $stack[] = $front;    
}
