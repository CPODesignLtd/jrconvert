...
//    $res = $res . "<td class='cell_zastavky_pasmo' style='vertical-align:top;text-align:center'>";
    ...
    $res = $res . "<label class='label'>" . iconv('ISO-8859-2', 'UTF-8', "Pásmo") . "</label>";
    $res = $res . "</div>";
...
//      $res = $res . $actual_time . " | " . $startcas . " | " . ($startcas + $incmin) . " | " . $index_vozu . " | " . $i;
      ...
      if ($index_vozu[$i] == TRUE) {
        if ($Linka->doprava == 'T') {
          $res = $res . "<img src='http://www.mhdspoje.cz/jrw50/image/autobus_small.png'>&nbsp;</img>";
        }
        if ($Linka->doprava == 'O') {
          $res = $res . "<img src='http://www.mhdspoje.cz/jrw50/image/trolejbus_small.png'>&nbsp;</img>";
        }
        if ($Linka->doprava == 'A') {
          $res = $res . "<img src='http://www.mhdspoje.cz/jrw50/image/tramvaj_small.png'>&nbsp;</img>";
        }
        if ($Linka->doprava == 'L') {
          $res = $res . "<img src='http://www.mhdspoje.cz/jrw50/image/lanovka_small.png'>&nbsp;</img>";
        }
      }
      }
...
//  $res = $res . "<td class = 'cell_zastavky_zastavka' style = 'vertical-align: top; text-align: center;'>";
  ...
  $res = $res . "<div class='div_ram'>";
  $res = $res . "<label class='label'>" . iconv('ISO-8859-2', 'UTF-8', "Zastávka") . "</label>";
  $res = $res . "</div>";
...
// -------- zahlavi c. spoje ----------
      ...
      for ($i = 0; $i < $JRType->odjezdy->size() + 1; $i++) {
        if ($i == $JRType->odjezdy->size()) {
          ...
          $res = $res . "<td class='cell_time_jr_zahlavi' title = '" . iconv('ISO-8859-2', 'UTF-8', "spoj èíslo : ") . $JRType->odjezdy->elementAt($i)->cspoje . "'>";
...
//  $res = $res . "<a id = 'header_zastavky_pasmo' class = 'header_zastavky_pasmo'>";
  $res = $res . "<label class='label'>" . iconv('ISO-8859-2', 'UTF-8', "Min") . "</label>";
//  $res = $res . "</a>";
  $res = $res . "</div>";
...
// --- Èas ---
  ...
  $res = $res . "<div class = 'div_ram'>";
//  $res = $res . "<a id = 'header_zastavky_pasmo' class = 'header_zastavky_pasmo'>";
  $res = $res . "<label class='label'>" . iconv('ISO-8859-2', 'UTF-8', "Èas") . "</label>";
//  $res = $res . "</a>";
  $res = $res . "</div>";
...
//  $res = $res . "<a id = 'header_zastavky_zastavka' class = 'header_zastavky_zastavka'>";
  $res = $res . "<label class='label'>" . iconv('ISO-8859-2', 'UTF-8', $rs_jr_zastavka/* "Zastávka" */) . "</label>";
//  $res = $res . "</a>";
  $res = $res . "</div>";
...
//    echo $showpolohu . " | " . $urcenipolohy . "-";
    $res = $res . getJRDiv($Linka, $Trasa, $TChrono, $tarif, $smer, ($smer == 0) ? $hasPasmoA : $hasPasmoB, $TPoznamky, $TJRTypes, $location, $packet, $jrtype, $x, $y, $TPoznamkyZastavky, $TPoznamkySpoje, $denniJR, $datumJR, $sdruzJR, $layout, $Packets, $zastavkysmery, $showpolohu, $urcenipolohy);
    $res = $res . '</div>';
