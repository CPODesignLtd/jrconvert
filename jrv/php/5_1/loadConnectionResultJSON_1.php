...
while ($row = $result->fetch_row()) {
    if (($row[0] != '') && ($row[1] != '')) {
        ...
        $pPrestup = new TPrestup();
        $pPrestup->create(-1, $row[1], $row[0], $row[2], $row[3]);
        $PESOBUS_ZASTAVKY[$row[1]][$row[0]] = $pPrestup;        
    }
}
...
nc.style.height = '0px';";
        ...
        if ($location == 17) {
            $res = $res . "<tr><td colspan='5' style='text-align: left;'>" . iconv('UTF-8', 'UTF-8', 'Délka cesty&nbsp;') . ($konec - $pocatek) . iconv('UTF-8', 'UTF-8', '&nbsp;min.') . "</td>".
                    "<td style='text-align: right;'><img src='//152.mhdspoje.cz/JRw50new/css/Plzen/ico-detail.png'>&nbsp<img src='//152.mhdspoje.cz/JRw50new/css/Plzen/ico-print.png'></td></tr>";
...
if ($location == 17) {
    ...
    $res = $res . /* $ppocatek . " - ". intval($pocatek / 60) . ":" . ($pocatek % 60) . */"</br><div>
<center>
<button title='Další spojení' onclick='Time.initialize(" . intval($pocatek / 60) . ", " . intval($pocatek % 60) . "); JR.spojeniResult(vlocation, vpacket, Time.getHH(), Time.getMM(), " . $prime . ", 5);' id='mod-rscontact-submit-btn-124' class='formButton'><span class=''></span><img src='css/Plzen/search_next.png'></img>Další spojení</button>
</center>
</div>";
}
