...
//              echo "cas + pommincekani = " . ($cas + $pommincekani) . " | " . "cas + maxprestup = " . ($cas + $maxprestup) . " \ " . $startCAS . "</br>";
                        ...
                        if (($od_zastavky == $pocatek) && ($do_zastavky == $cil) && ($H * 60 + $M <= $startCAS)) {
//                echo $c_linky . " | " . $od_zastavky . " -> " . $do_zastavky . " : " . $startCAS . " -> " . $endCAS . " ( " . $doba_jizdy . " )</br>";
                $pomPartSpojeni = new TPartSpojeni();
                $pomPartSpojeni->c_linky = $c_linky;
                $pomPartSpojeni->c_spoje = $pomSpojItem->c_spoje;
                $pomPartSpojeni->od_zastavky = getZASTAVKA($od_zastavky);
                $pomPartSpojeni->do_zastavky = getZASTAVKA($do_zastavky);
                $pomPartSpojeni->odjezd = $startCAS;
                $pomPartSpojeni->prijezd = $endCAS;
                $HH = (int) (($startCAS / 60) % 24);
                $MM = (int) ($startCAS % 60);
                $pomPartSpojeni->text_odjezd = (($HH < 10) ? '0' . $HH : $HH) . ':' . (($MM < 10) ? '0' . $MM : $MM);
                $HH = (int) (($endCAS / 60) % 24);
                $MM = (int) ($endCAS % 60);
                $pomPartSpojeni->text_prijezd = (($HH < 10) ? '0' . $HH : $HH) . ':' . (($MM < 10) ? '0' . $MM : $MM);
                $pomPartSpojeni->smer = $smer;
                $pomPartSpojeni->nazev_linky = getLINKA($c_linky)->nazev_linky;
                $pomPartSpojeni->typ_dopravy = getLINKA($c_linky)->doprava;
                $primespoje[count($primespoje)] = $pomPartSpojeni;
              }
...
//    }
  ...
  for ($i = 0; $i < count($primespoje); $i++) {
    $pomSpojeni = new TSpojeni();
    $pomSpojeni->cislo_spojeni = count($pommyarraylist) + 1;
    $pomSpojeni->cislo_spoje = $primespoje[$i]->c_spoje;
    $pomSpojeni->PartSpoje[0] = $primespoje[$i];
    $pommyarraylist[count($pommyarraylist)] = $pomSpojeni;
  }

  for ($i = 0; $i < count($pommyarraylist); $i++) {
    for ($ii = 0; $ii < count($pommyarraylist) - 1; $ii++) {
      $odjezd = $pommyarraylist[$ii]->PartSpoje[0]->odjezd * 10000000000;
      $prijezd = $pommyarraylist[$ii]->PartSpoje[count($pommyarraylist[$ii]->PartSpoje) - 1]->prijezd * 1000000;
      $odjezdnext = $pommyarraylist[$ii + 1]->PartSpoje[0]->odjezd * 10000000000;
      $prijezdnext = $pommyarraylist[$ii + 1]->PartSpoje[count($pommyarraylist[$ii]->PartSpoje) - 1]->prijezd * 1000000;
      $cas = $odjezd + $prijezd + $pommyarraylist[$ii]->PartSpoje[0]->c_linky;
      $casnext = $odjezdnext + $prijezdnext + $pommyarraylist[$ii + 1]->PartSpoje[0]->c_linky;
      if ($casnext < $cas) {
        $p = $pommyarraylist[$ii];
        $pommyarraylist[$ii] = $pommyarraylist[$ii + 1];
        $pommyarraylist[$ii + 1] = $p;
      }
    }
  }

  $myarraylist = null;
  if (count($pommyarraylist) > 0) {
    $myarraylist[0] = $pommyarraylist[0];
    $myarraylist[0]->cislo_spojeni = count($myarraylist);
  }
  for ($i = 1; $i < count($pommyarraylist); $i++) {
    $odjezd = $pommyarraylist[$i]->PartSpoje[0]->odjezd;
    $prijezd = $pommyarraylist[$i]->PartSpoje[count($pommyarraylist[$i]->PartSpoje) - 1]->prijezd;
    $linka = $pommyarraylist[$i]->PartSpoje[0]->c_linky;
    $exist = FALSE;
    for ($ii = 0; $ii < count($myarraylist); $ii++) {
      if (($myarraylist[$ii]->PartSpoje[0]->odjezd == $odjezd) && ($myarraylist[$ii]->PartSpoje[count($myarraylist[$ii]->PartSpoje) - 1]->prijezd == $prijezd) && ($myarraylist[$ii]->PartSpoje[0]->c_linky == $linka)) {
        $exist = TRUE;
      }
    }
    if ($exist == FALSE) {
      $myarraylist[count($myarraylist)] = $pommyarraylist[$i];
      $myarraylist[count($myarraylist) - 1]->cislo_spojeni = count($myarraylist);
    }
  }
...
$sql = "select c_linky, smer, HH, MM, chrono, c_spoje from spoje where spoje.idlocation = " . $location . " and spoje.packet = " . $packet . " and ((spoje.kodpozn & " . $varGRF . ") > 0 or spoje.kodpozn = 0)  AND spoje.voz = 1 AND (spoje.vlastnosti & 2048) <> 2048";
$result = $mysqli->query($sql);
...
$sql = "select chronometr.c_linky, chronometr.smer, chronometr.chrono, chronometr.c_tarif, chronometr.doba_pocatek, chronometr.c_zastavky, zaslinky.prestup, zaslinky.zast_a as A, zaslinky.zast_b as B from chronometr
  left outer join linky on (linky.c_linky = chronometr.c_linky and linky.idlocation = chronometr.idlocation and linky.packet = chronometr.packet)
...
where chronometr.idlocation = " . $location . " and chronometr.packet = " . $packet . "  and chronometr.doba_jizdy > -1 and linky.jr_od <= '" . date_format(new DateTime($datumJR), 'Y-m-d') . "' and linky.jr_do >= '" . date_format(new DateTime($datumJR), 'Y-m-d') . "'";
$result = $mysqli->query($sql);
...
for ($i = 0; $i < count($myarraylist); $i++) {
    ...
    for ($ii = 0; $ii < count($myarraylist[$i]->PartSpoje); $ii++) {
        $val1 = $myarraylist[$i]->PartSpoje[$ii];
        $val2 = new TSpojPart();
        $val2->c_linky = $val1->c_linky;
        $val2->do_zastavky = $val1->do_zastavky->id_zastavky;
        $val2->doprava = $val1->typ_dopravy;
        $val2->nazev_linky = $val1->nazev_linky;
        $val2->od_zastavky = $val1->od_zastavky->id_zastavky;
        $val2->odjezd = $val1->odjezd;
        $val2->prijezd = $val1->prijezd;
        $val2->od_nazev = "";
        $val2->do_nazev = "";
        $val2->cislo_spoje = $myarraylist[$i]->cislo_spojeni - 1;
        $val2->c_spoje = $val1->c_spoje;
