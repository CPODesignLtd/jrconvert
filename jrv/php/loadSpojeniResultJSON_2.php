...
//  $sql = "call getOdjezdyZastavka1(" . $location . ", " . $packet . ", " . (($c_hladiny <= 1) ? (/* $max */$min) : ($min + $dobaPrestup)) . ", " . (($c_hladiny <= 1) ? (/* $min */$max) : ($max + $dobaSpoje)) . ", " . $cil . ", " . $varGRF . ", '" . getVarray($Vod) . "', '', '');";  
  ...
  do {
    if ($vysledek = $mysqli->store_result()) {
      while ($data1 = $vysledek->fetch_row()) {
        ...
        if ($reshladiny[$c_hladiny][(int) $data1[2]][$data1[0]][$data1[8]][$data1[5]] == NULL) {
          $data = new Vector();
          $reshladiny[$c_hladiny][(int) $data1[2]][$data1[0]][$data1[8]][$data1[5]] = $data;
//      echo 'tvorim vector</br>';
        }

        $data = $reshladiny[$c_hladiny][(int) $data1[2]][$data1[0]][$data1[8]][$data1[5]];
        $hrana = new TSpojPart();
        $hrana->linka = $data1[0];
        $hrana->Zod = $data1[1];
        $hrana->Zdo = $data1[2];
...
$hladiny[0][$pocatek][0][0][0] = new Vector();
$hladiny[0][$pocatek][0][0][1] = new Vector();
...
while (list($index, $stav) = each($hladiny)) {
  ...
  while (list($index1, $stav1) = each($hladiny[$index])) {
    ...
    while (list($index2, $stav2) = each($hladiny[$index][$index1])) {
      echo '           --- linka --- ' . $index2 . '</br>';
      while (list($index3, $stav3) = each($hladiny[$index][$index1][$index2])) {
        echo '           --- dist --- ' . $index3 . '</br>';
        for ($sm = 0; $sm < 2; $sm++) {
          echo '               --- smer --- ' . $sm . '</br>';

          if ($hladiny[$index][$index1][$index2][$index3][$sm] != NULL) {
//        echo 'velikost = '.$hladiny[$index][$index1][$sm]->size().'</br>';
            for ($i = 0; $i < $hladiny[$index][$index1][$index2][$index3][$sm]->size(); $i++) {
              echo $hladiny[$index][$index1][$index2][$index3][$sm]->elementAt($i)->linka . ' , ' . $hladiny[$index][$index1][$index2][$index3][$sm]->elementAt($i)->Zod . ' , ' . $hladiny[$index][$index1][$index2][$index3][$sm]->elementAt($i)->Zdo . ' , ' . $hladiny[$index][$index1][$index2][$index3][$sm]->elementAt($i)->CASod . ' - ' . $hladiny[$index][$index1][$index2][$index3][$sm]->elementAt($i)->CASdo . '  ->  </br>';
            }
          }
        }
      }
