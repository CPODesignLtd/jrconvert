...
var mesiceW1250SK = new Array("janu·ra","febru·ra","mareca","aprÌla","m·ja","j˙na","jula","augusta","septembera","oktÛbera","novembera","decembera");
...
function JRKalendar(aDivDatum, aTextDatum, aKalendar, other, alang) {
  this.self = this;
  if (alang == null) {
    this.lang = 'cz';
  } else {
...
function getJR(linka, smer, tarif, location, packet, denJR, datum, sdruzJR, sloupec, x, y, sel, adeleteonshow, packets, showkurz, incspoje) {
  if (showkurz == null) {
    showkurz = 0;
  }
  if (showkurz != 1) {
  ...
  if ((x == null) || (y == null) || (sloupec == null)) {
    ...
    "&packet=" + packet + "&datum=" + datum + "&denni=" + denJR + "&sdruz=" + sdruzJR + "&sel=" + sel + "&packets=" + packets + "&kurz=" + showkurz + "&incspoje=" +incspoje  + "&callback=getJRData";
  } else {
    fullUrl = /*"http://www.mhdspoje.cz/jrw50/php/*/path + "loadJRJSON.php?linka=" + linka +
    "&smer=" + smer + "&tarif=" + tarif + "&location=" + location +
    "&packet=" + packet + "&denni=" + denJR + "&datum=" + datum + "&sdruz=" + sdruzJR + "&sel=" + sel + "&jrtype=" + sloupec + "&x=" + x + "&y=" + y + "&packets=" + packets + "&kurz=" + showkurz + "&incspoje=" +incspoje  + "&callback=getJRData";
  }
  scriptObj.setAttribute("src", fullUrl/* + noCacheIE*/);
...
function getSeznamZastavkaJR(location, packet, datum, idzastavky) {
  ...
  scriptObj.setAttribute("charset", "windows-1250");
  noCacheIE = '&noCacheIE=' + (new Date()).getTime();
  fullUrl = "//www.mhdspoje.cz/jrw50/php/loadSeznamZastavkaJSON.php?location=" + location + "&packet=" + packet + "&datum=" + datum + "&idzastavka=" + idzastavky  + "&callback=getSeznamZastavkaJRData";
  scriptObj.setAttribute("src", fullUrl/* + noCacheIE*/);
...
JRData.prototype.getLinkyList = function(location, packet, comboName) {
  ...
  scriptObj.setAttribute("charset", "windows-1250");
  noCacheIE = '&noCacheIE=' + (new Date()).getTime();
  fullUrl = "//www.mhdspoje.cz/jrw50/php/ListLinkyJSON.php?location=" + location + "&packet=" + packet + "&datum=" + this.kalendar.d + "_" + this.kalendar.m + "_" + this.kalendar.y + "&target=" + comboName +  "&callback=getLinkyData";
  scriptObj.setAttribute("src", fullUrl/* + noCacheIE*/);
...
JRData.prototype.getRouteData = function(data) {
  ...
  for (i = 0; i < pp.length; i++) {
    ...
    var marker = new google.maps.Marker({
      ...
      icon: "//www.mhdspoje.cz/jrw50/image/mark" + (i + 1) + ".png"
    });
